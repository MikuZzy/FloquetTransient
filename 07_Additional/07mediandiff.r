...
# Difference in median expression level from two temperatures are varied. This file is run on HPC
...
# varied median expressional level difference
mediandiff <- -par_value[4]-par_value[6]-par_value[7]
newpar6 <- seq(-par_value[4]-par_value[7],par_value[6],length.out=11)

FNs_nosel <- matrix(data=NA, nrow=11, ncol=16)
...
FNs_withsel <- matrix(data=NA, nrow=11, ncol=21)
...
for (k in seq(11)) {
  ...
  newpar <- c(par_value[1:5],newpar6[k],par_value[7:9])
  fitted_value <- func_fullmodel_type(newpar,time_comb,type=16)
  ...
  for (i in seq(1000)) {
    res_value <- sample(res,size=n,replace=TRUE)
    temp_data <- data.frame(
      'value' = fitted_value + res_value,
      'time' = time_comb
    )
    #######
...
write.csv(FNs_nosel, file = '07mediandiff_nosel.csv')
write.csv(FNs_withsel, file = '07mediandiff_withsel.csv')
save.image(file = '07mediandiff.Rdata')
